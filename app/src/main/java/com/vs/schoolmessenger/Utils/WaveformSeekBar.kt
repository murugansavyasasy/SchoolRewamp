...
class WaveformSeekBar @JvmOverloads constructor(
    ...
    private var waveSpeed = 0.1f
    private var waveFrequency = 0.5f
    private var baseAmplitude = 100f
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        barWidth = (width - (numberOfBars - 1) * barSpacing) / numberOfBars
        val centerY = height / 2f

        for (i in 0 until numberOfBars) {
            val normalizedIndex = i / numberOfBars.toFloat()
            val barHeight =
                currentAmplitude * sin(normalizedIndex * waveFrequency * 2 * Math.PI + wavePhase).toFloat()
            val adjustedHeight = max(10f, abs(barHeight)) // Ensure a minimum height

            val left = i * (barWidth + barSpacing)
            val top = centerY - adjustedHeight / 2
            val right = left + barWidth
            val bottom = centerY + adjustedHeight / 2

            val colorLevel = abs(barHeight) / baseAmplitude
            barPaint.color = Color.rgb(0, (colorLevel * 255).toInt(), 255)

            canvas.drawRect(left, top, right, bottom, barPaint)
        }

        postDelayed(updateRunnable, 16L) // Approximately 60 FPS
    }
