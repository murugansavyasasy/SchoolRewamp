...
object VimeoVideoUpload {
    ...
    private fun createVimeoUploadURL(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val sizeInBytes = getFileSize(activity, videoFilePath)
    ...
    private fun uploadVideoToVimeo(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                val videoLength = getFileSize(activity, videoFilePath)
                ...
                // KEY FIX: Enable chunked streaming mode to prevent buffering entire file in memory
                conn.setChunkedStreamingMode(0)
                conn.doOutput = true

                val outputStream = conn.outputStream

                val buffer = ByteArray(64 * 1024) // Increased to 64 KB buffer
                var bytesRead: Int
                ...
                inputStream.use { input ->
                    outputStream.use { output ->
                        while (true) {
                            ...
                            output.flush() // Flush after each chunk to keep memory low
    ...
    private fun getFileSize(activity: Activity, videoFilePath: String): Long {
        return if (videoFilePath.startsWith(Constant.content)) {
